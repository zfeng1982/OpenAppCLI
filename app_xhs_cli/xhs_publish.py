...
def add_topic_via_button(driver, topic_keyword,move=True, hint="添加正文或发语音"):
    ...
    try:
        topic_item = WebDriverWait(driver, 10).until(
            EC.presence_of_element_located((AppiumBy.XPATH, target_xpath))
        )
        topic_item.click()
        print(f"✅ 已添加话题：{topic_keyword}")
        time.sleep(0.5)
    except Exception as e:
        print(f"⚠️ 新增话题失败'{topic_keyword}'")
        print(f"发生未知错误: {e}")
        # print(f"⚠️ 未找到推荐话题 '{topic_keyword}'，删除话题")
        # # 删除输入的 '#' + 关键词
        # for _ in range(len(topic_keyword) + 1):
...
def click_write_idea(driver,idea:str,title_text="", body_text="", topics=None):
    """点击“写想法”按钮"""
    idea_btn = WebDriverWait(driver, 10).until(
        EC.element_to_be_clickable((AppiumBy.XPATH, "//android.widget.TextView[@text='写想法']"))
    )
    idea_btn.click()
    print("✅ 已点击“写想法”")
    try:
        # 等待并定位到编辑框（对应 XML 中的 EditText）
        idea_input = WebDriverWait(driver, 10).until(
        ...
        for attempt in range(max_retries):
            try:
                # 每次重新查找所有可点击的卡片
                cards = driver.find_elements(AppiumBy.XPATH,"//androidx.recyclerview.widget.RecyclerView//android.view.ViewGroup[@clickable='true']")
                if not cards:
                    raise Exception("未找到任何卡片")
                chosen = random.choice(cards)
                # 尝试点击
                chosen.click()
                print(f"已随机选择卡片 (尝试 {attempt + 1}/{max_retries})")
                break  # 成功则退出重试
            except StaleElementReferenceException as e:
                print(f"卡片元素过期，重试 {attempt + 1}/{max_retries}...")
                time.sleep(0.5)
                if attempt == max_retries - 1:
                    raise e  # 最后一次仍失败则抛出异常
